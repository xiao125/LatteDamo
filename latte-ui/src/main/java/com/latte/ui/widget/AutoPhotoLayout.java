...
public final class AutoPhotoLayout extends LinearLayoutCompat {
    ...
    public AutoPhotoLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mMaxLineNum = typedArray.getInt(R.styleable.camera_flow_layout_line_count,3);//默认一张3张
        mImageMargin = typedArray.getInt(R.styleable.camera_flow_layout_item_margin,0);//默认0
        mIconSize = typedArray.getDimension(R.styleable.camera_flow_layout_icon_size,20);//默认20间隙
        typedArray.recycle();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i=0;i<cCount;i++){
            final View child = getChildAt(i); //子View
            //测量子View的宽和高
            ...
            if (lineWith +childWidth >sizeWith-getPaddingLeft()-getPaddingRight()){
                ...
                //重置lineWidth
                lineWith = childWidth;
                height +=lineHeight; //换行高度就增加了，比如两行。。
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i=0;i<cCount;i++){
            ...
            lineWidth +=childWith + lp.leftMargin + lp.rightMargin;//每一行宽度
        ...
        final int lineNum = ALL_VIEWS.size();//总行数
        for (int i=0;i<lineNum;i++){
            //当前行所有的View
            mLineViews = ALL_VIEWS.get(i);
            lineHeight = LINE_HEIGHTS.get(i);
            final int size = mLineViews.size();//每一行里面的子view数量
            ...
            left = getPaddingLeft();
            top +=lineHeight; //每换一行后的高度就等于累加上一行高度
        }
    ...
    private void initAddIcon(){
        ...
        mIconAdd.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                mDelegate.startCameraWithCheck(); //点击加号，进行添加图片
            }
        });
